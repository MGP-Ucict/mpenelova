					...
					@endif
	   				<div class="grid xs:grid-cols-1 sm:grid-cols-1 md:grid-cols-4 lg:grid-cols-4 border-b-2 flex">
						<div class="column-xs mb-2">#</div>
					  	...
					  	<div class="column-xs mb-2">{{ __("Email") }}</div>
					  	<div class="column-xs mb-2">{{ __("Actions") }}</div>
					</div>
								      ...
								      <el-dialog-panel class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all data-closed:translate-y-4 data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in sm:my-8 sm:w-full sm:max-w-lg data-closed:sm:translate-y-0 data-closed:sm:scale-95">
									    ...
									    <form action="{{ route('users.destroy', $user)}}" method="post">
									        ...
									        <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
									         <button type="submit" command="close" commandfor="dialog-{{$user->id}}" class="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm text-white shadow-xs hover:bg-red-500 sm:ml-3 sm:w-auto"> {{ __("Delete") }} </button>
									          <button type="button" command="close" commandfor="dialog-{{$user->id}}" class="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm text-gray-900 shadow-xs inset-ring inset-ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto">{{ __("No") }}</button>
									        </div>
									    </form>
								      </el-dialog-panel>
								    </div>
