					...
					@endif
	   				<div class="grid xs:grid-cols-1 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-3 border-b-2">
						<div class="column-xs mb-2">#</div>
					  	<div class="column-xs mb-2" >{{ __("Name") }}</div>
					  	<div class="column-xs mb-2 w-80 flex">{{ __("Actions") }}</div>
					</div>
								      ...
								      <el-dialog-panel class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all data-closed:translate-y-4 data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in sm:my-8 sm:w-full sm:max-w-lg data-closed:sm:translate-y-0 data-closed:sm:scale-95">
								        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
								          <div class="sm:flex sm:items-start">
								            ...
								            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
								              ...
								              <div class="mt-2">
								                <p class="text-sm text-gray-500">{{ __("Really delete") }} {{$role->name}}?</p>
								              </div>
								            </div>
								          </div>
								        </div>
									    <form action="{{ route('roles.destroy', $role)}}" method="post">
									        ...
									        <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
									         <button type="submit" command="close" commandfor="dialog-{{$role->id}}" class="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm text-white shadow-xs hover:bg-red-500 sm:ml-3 sm:w-auto">{{ __("Delete") }}</button>
									          <button type="button" command="close" commandfor="dialog-{{$role->id}}" class="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm text-gray-900 shadow-xs inset-ring inset-ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto">{{ __("No") }}</button>
									        </div>
									    </form>
								      </el-dialog-panel>
								    </div>
