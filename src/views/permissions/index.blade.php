                		...
                		<div class="flex max-w-sm mb-4">
							...
							<a href="{{ route('permissions.create')}}"
					        	class="bg-gray-700 text-slate-50 rounded-full p-2 flex xs:w-full sm:w-full md:w-60 lg:w-60 text-sm justify-center mb-4">
									...
									{{trans('lang::translation.CreatePermission')}}
						     </a>
					    </div>
					</div>
	            	@if (session('status'))
					...
					@foreach($permissions as $permission)
						<div class="grid xs:grid-cols-1 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-3 border-b-2 flex mb-4">
							...
							<div class="column-xs mt-2 mb-2">
								...
								{{trans('lang::translation.Delete')}}</button>
								      ...
								      <el-dialog-panel class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all data-closed:translate-y-4 data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in sm:my-8 sm:w-full sm:max-w-lg data-closed:sm:translate-y-0 data-closed:sm:scale-95">
								        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
								          <div class="sm:flex sm:items-start">
								            ...
								            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
								              ...
								              <div class="mt-2">
								                <p class="text-sm text-gray-500">{{trans('lang::translation.ReallyDelete')}} {{$permission->name}}?</p>
								              </div>
								            </div>
								          </div>
								        </div>
									    <form action="{{ route('permissions.destroy', $permission)}}" method="post">
									        ...
									        <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
									         <button type="submit" command="close" commandfor="dialog-{{$permission->id}}" class="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm text-white shadow-xs hover:bg-red-500 sm:ml-3 sm:w-auto">{{trans('lang::translation.Delete')}}</button>
									          <button type="button" command="close" commandfor="dialog-{{$permission->id}}" class="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm text-gray-900 shadow-xs inset-ring inset-ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto">{{trans('lang::translation.No')}}</button>
									        </div>
									    </form>
								      </el-dialog-panel>
								    </div>
